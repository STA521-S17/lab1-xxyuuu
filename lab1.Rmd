...
```{r}
coplot2 <-ggplot(data=College, aes(y= Outstate, x=Elite, color = factor(Private)))
coplot2 <- coplot2 + geom_point(size = 0.5)
coplot2 <- coplot2 + geom_boxplot(alpha = 0.5)
coplot2
```
```{r}
coplot3 <-ggplot(data=College, aes(y= Outstate, x=Elite, alpha=0.5))
coplot3 <- coplot3 + geom_point(size=0.5)
coplot3 <- coplot3 + geom_boxplot()
coplot3 <- coplot3 + facet_wrap(~Private)
coplot3
```
